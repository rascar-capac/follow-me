...
public class AmbiantManagerTEST : Singleton<AmbiantManager>
{
    ...
    public float DayTimeTransitionMaxProgressPerSecond;
    ...
    public IEnumerator ModifyCycleProgress(int targetPhaseIndex)
    {
        ...
        while(PhaseIndex != targetPhaseIndex || IsAlreadyInTargetPhase)
        {
            if(Application.isPlaying)
            {
                ...
                //SkyboxCycleManager.Instance.CycleProgress += DayTimeTransitionSpeedCurve.Evaluate() * DayTimeTransitionMaxProgressPerSecond * Time.deltaTime;
                SkyboxCycleManager.Instance.CycleProgress += Mathf.SmoothDamp(dayTime, targetDayTime, ref currentVelocity, DayTimeTransitionDuration, DayTimeTransitionMaxProgressPerSecond);
