  ...
  //      _SpontaneityCheckTimer = ComputeRandomSpontaneityCheckTimer();
    ...
    */
    ...
    public IEnumerable RotatingAround(Transform t, float duration = 0f, float speedMove = 0)
    {
       ...
       // float startRotation = transform.rotation;
        while (true)
        {
    ...
    public IEnumerator GoingToPosition(Vector3 position, float WaitAndComeBackSeconds = 0f, bool ChangeRotation = true, float speedMove = 0f, RotationDirection dir = RotationDirection.None)
    {
        ...
        while (Vector3.Distance(transform.position, position) > 250f)
        {
            if (ChangeRotation)
            {
                ...
                slerpTimer += (Time.deltaTime / AngularSpeed);
                RotateTowards(startRotation, position, slerpTimer, dir);
        ...
        if (WaitAndComeBackSeconds > 0)
        {
            ...
            while (Vector3.Distance(transform.position, initialPosition) > 0.1f)
            {
                ...
                yield return null;
                if (ChangeRotation)
                    RotateTowards(startRotation, initialPosition, slerpTimer, dir);
            }
        }
    }
    public void RotateTowards(Quaternion initialRotation, Vector3 direction, float slerpTimer, RotationDirection Rdirection = RotationDirection.Left)
    {
...
=======
        ...
        transform.rotation = Quaternion.Slerp(initialRotation, lookRotation, slerpTimer);
>>>>>>> Stashed changes
